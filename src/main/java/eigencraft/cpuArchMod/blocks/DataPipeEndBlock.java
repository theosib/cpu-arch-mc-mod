...
import eigencraft.cpuArchMod.simulation.SimulationEndPipe;
...
public class DataPipeEndBlock extends Block {
    public DataPipeEndBlock(Settings settings) {
        super(settings);
    }
        ...
        if (!world.isClient){
            ...
            ((SimulationMasterProvider)world).getSimulationMaster().getIOManager().addSimulationTickRunnable(new SimulationIOManager.SimulationTickRunnable() {
                ...
                public void run(SimulationWorld simulationWorld) {
                    ...
                    simulationWorld.addSpecialPipe(new SimulationEndPipe(pos),pos);
                }
            });
        }
    }
